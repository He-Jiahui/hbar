...
description: 在 hbar 仓库开发、重构、评审和交付 React/Zustand 工作台、Bun TypeScript Host、SDK 或插件时使用。执行模块边界、MVVM/MVC、描述文件布局、就近交互、可靠增量同步、严格类型与 lint、逐功能验证和 Git 提交规范。仅用于 hbar 项目及其工作树。
---
...
4. 所有代码任务读取 [质量与交付](references/quality.md)，完成对应检查并及时提交。纯规范修改执行文档与技能校验，不虚构业务测试结果。
...
- 一个功能完成并通过验证后及时独立 commit；工作树和暂存区保持可解释，不混入产物、草稿或无关改动。
