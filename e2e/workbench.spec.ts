...
test('system terminal opens as an xterm workbench surface', async ({ page, context }) => {
  ...
  try {
    ...
    await shellInput.pressSequentially('echo HBAR_XTERM_E2E')
    await shellInput.press('Enter')
    await expect(terminal.locator('.xterm-screen')).toContainText('HBAR_XTERM_E2E')
    await page.keyboard.press('Control+Shift+P')
    ...
    const allTerminalPanels = page.locator('.system-terminal-panel')
    await expect(allTerminalPanels).toHaveCount(2)
    await expect(allTerminalPanels.filter({ visible: true })).toHaveCount(1)
    await expect(page.locator('.flexlayout__border_bottom [role="tab"]')).toHaveCount(2)
    await page.getByRole('tab', { name: '系统终端 1', exact: true }).click()
    await expect(allTerminalPanels.filter({ visible: true })).toHaveCount(1)
    await page.screenshot({ path: `artifacts/${Date.now()}-desktop-system-terminal.png` })
    const terminalPanels = allTerminalPanels.filter({ visible: true })
    await terminalPanels.getByRole('button', { name: '关闭系统终端', exact: true }).click()
    await expect(allTerminalPanels.filter({ visible: true })).toHaveCount(1)
    await allTerminalPanels.filter({ visible: true }).getByRole('button', { name: '关闭系统终端', exact: true }).click()
    await expect(allTerminalPanels).toHaveCount(0)
    await page.setViewportSize({ width: 390, height: 844 })
    await openMobileMore(page)
