...
test('model picker and permission preset survive a refresh', async ({ page, context }) => {
  ...
  try {
    ...
    await modelPage.getByRole('textbox', { name: '搜索供应商或模型', exact: true }).fill('fixture')
    const fixtureModel = modelPage.locator('.model-row').filter({ hasText: 'fixture' })
    await expect(fixtureModel).toBeVisible()
    await fixtureModel.getByRole('button', { name: /当前使用|使用/, exact: true }).click()
