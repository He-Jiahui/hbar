...
test('workbench keeps tools on demand and exposes the global command palette', async ({ page, context }) => {
  ...
  try {
    ...
    await expect(page.locator('.rb-activity-panel').filter({ visible: true }).locator('.activity-panel-glass')).toHaveClass(/glass-surface/)
    await activityRail.click()
    ...
    const planTab = page.getByRole('tab', { name: '计划', exact: true }).filter({ visible: true })
...
test('model picker and permission preset survive a refresh', async ({ page, context }) => {
  ...
  try {
    ...
    const modelMenu = page.getByRole('menu', { name: '选择模型', exact: true })
    await expect(modelMenu.locator('.rb-menu-glass')).toHaveClass(/glass-surface/)
    await expect(modelMenu.getByRole('textbox', { name: '搜索模型', exact: true })).toBeVisible()
    await modelMenu.getByRole('textbox', { name: '搜索模型', exact: true }).fill('fixture')
    await expect(modelMenu.getByRole('menuitemradio', { name: /Local fixture/ })).toBeVisible()
    await modelMenu.getByRole('menuitemradio', { name: /Local fixture/ }).click()
...
test('phone supports send, denial, files and model settings without horizontal overflow', async ({ page, context }) => {
  ...
  try {
    ...
    await expect(page.locator('.rb-file-nav').filter({ visible: true }).locator('.file-nav-glass')).toHaveClass(/glass-surface/)
