...
test('model picker exposes provider, model and thinking level hierarchy', async ({ page, context }) => {
  ...
  try {
    ...
    await expect(menu.locator('.model-picker-body')).toHaveCSS('grid-template-columns', /\S+ \S+/)
    ...
    await deep.click()
    const detail = menu.getByLabel('UI Model Catalog / Deep model 详情', { exact: true })
    await expect(detail).toBeVisible()
    await expect(detail).toContainText('上下文')
    await expect(detail).toContainText('最大输出')
    await expect(detail.getByRole('group', { name: 'Deep model 思考等级', exact: true })).toBeVisible()
    const high = detail.getByRole('radio', { name: '高', exact: true })
    ...
    await page.setViewportSize({ width: 390, height: 844 })
    await picker.click()
    const mobileMenu = page.getByRole('menu', { name: '选择模型', exact: true })
    await expect(mobileMenu.locator('.model-picker-body')).toHaveCSS('grid-template-columns', /\S+/)
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBeTruthy()
  } finally {
